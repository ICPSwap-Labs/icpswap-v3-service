...
cat > dfx.json <<- EOF
{
  "canisters": {
    ...
    "SwapDataBackup": {
      "main": "./src/backup/SwapDataBackup.mo",
      "type": "motoko"
    },
    "PasscodeManager": {
...
echo "==> swapDataBackupId (\"$swapDataBackupId\")"
...
{   
    ...
    echo "==> backup data and recover"
    # backup data and recover
    result=`dfx canister call $swapDataBackupId backupAndRecoverPool "(principal \"$poolId\")"`
    echo "backup and recover result: $result"
    backUpCid=$(echo "$result" | sed -e 's/[(")]//g')
    # transfer token to recover canister
    result=`dfx canister call $poolId transferAll "(principal \"$backUpCid\")"`
    # reset pool id
    poolId="$backUpCid"
    dfx canister call $dipAId approve "(principal \"$poolId\", $TOTAL_SUPPLY)"
    dfx canister call $dipBId approve "(principal \"$poolId\", $TOTAL_SUPPLY)"
